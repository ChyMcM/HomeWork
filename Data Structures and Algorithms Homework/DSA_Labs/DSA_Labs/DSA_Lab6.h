...
public:
	...
	int GetLetterValue(char _letter) const {
		// TODO: Implement this method
		return mLetterValues[(int)_letter - 65];
	}
	...
	std::pair<std::string, int> CreatePair(const std::string& _word) const {
		// TODO: Implement this method
		int val = GetWordValue(_word);
		std::pair<std::string, int> io;
		io.first = _word;
		io.second = val;
		return io;
	}
	...
	void LoadWords(const char* _filename) {
		// TODO: Implement this method
		std::ifstream words(_filename);
		std::string line;
		while (!words.eof())
		{
			for (line; getline(words, line); )
			{
				mScrabbleMap.insert(CreatePair(line));
			}
		}
		words.close();
	}
	...
	int FindValueInMap(const std::string& _word) {
		// TODO: Implement this method
		auto it = mScrabbleMap.find(_word);
		if (it != mScrabbleMap.end())
		{
