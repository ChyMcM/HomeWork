...
void DSA_TestSuite_Lab7::BSTRemoveCase1RootLeft() {
	...
	if (bst->mRoot->parent != nullptr)
		PrintMessage("Root's parent was not set to null");

	// Testing for success
	if (metrics.inUse == memoryUsed - sizeof(myNode) &&
		nullptr == bst->mRoot->parent && bst->mRoot == val25)
		PrintMessage();
...
void DSA_TestSuite_Lab7::BSTRemoveCase1RootRight() {
	...
	// Testing for success
	if (metrics.inUse == memoryUsed - sizeof(myNode) &&
		nullptr == bst->mRoot->parent && bst->mRoot == val75)
		PrintMessage();
...
void DSA_TestSuite_Lab7::BSTRemove() {
	...
	bool case0Removed = bstCase0->Remove(case0ToRemove);
