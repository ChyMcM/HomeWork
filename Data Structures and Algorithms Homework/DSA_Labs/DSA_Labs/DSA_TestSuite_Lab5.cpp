...
void DSA_TestSuite_Lab5::DictInsertOverwrite() {
	...
	// Error tests
	if (dict.mTable[correctBucket].size() != 1)
		PrintMessage("New pair was added, rather than updating existing value");
	if (!found)
		PrintMessage("Data was not found in correct bucket");
	if (foundPair.key != floatKey)
		PrintMessage("Key has incorrect value");
	...
	if (found &&
		...
		dict.mTable[correctBucket].size() == 1)
		PrintMessage();
}
...
void DSA_TestSuite_Lab5::DictFind() {
	...
	// Error testing
	if (resultVal != valueToSearchFor * 2)
