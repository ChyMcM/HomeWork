...
class Huffman {
	...
	void GenerateTree() {
		...
		while (queue.size() != 1)
		{
			HuffNode* left1 = queue.top();
			queue.pop();
			HuffNode* right1 = queue.top();
			queue.pop();
			HuffNode* parent = new HuffNode(-1, (right1->freq + left1->freq), left1, right1);
			left1->parent = parent;
	...
	void GenerateEncodingTable() {
		...
		for (size_t i = 0; i < mLeafList.size(); i++)
		{
			...
			while (helper->parent != nullptr)
			{
				...
				else if (helper->parent->right == helper)
				{
	...
	void Compress(const char* _outputFile) {
		...
		BitOStream neo(_outputFile, (const char*)mFrequencyTable, sizeof(int) * 257);
		...
		{
			neo << mEncodingTable[(unsigned char)mew.get()];
		}
	...
	void Decompress(const char* _outputFile) {
		...
		BitIStream str(mFileName.c_str(), (char*)mFrequencyTable, sizeof(int) * 257);
		...
		ofstream mew(_outputFile, std::ios::binary);
		...
		for (size_t i = 0; i < mFrequencyTable[256]; i++)
		{
			while (start->right != nullptr && start->left != nullptr)
			{
				str >> move;
				if (move == true)
				{
